...
import { BookOpen, Music, MessageCircle, CheckCircle, XCircle, Play, Pause, Star, Heart, Send, User, Lock } from 'lucide-react';
...
const vocabList = [
  ...
  { word: "left", phonetic: "[left]", type: "n.", meaning: "左边, 左转弯" },
            ...
            {!isAnswered ? (
              <button
                ...
                className={`mt-4 w-full py-2 rounded-lg font-bold text-sm transition-all ${
                  answers[q.id] ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-200 text-gray-400'
                }`}
              >
                提交答案
              </button>
            ) : (
              <div className={`mt-4 p-3 rounded-lg text-sm ${isCorrect ? 'bg-green-50' : 'bg-red-50'}`}>
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex bg-white rounded-lg p-1 shadow-sm">
        <button
          ...
          className={`flex-1 py-2 text-sm rounded font-medium ${tab === 'question' ? 'bg-blue-100 text-blue-700' : 'text-gray-500'}`}
        >
          ❓ 问题留言
        </button>
        <button
          onClick={() => setTab('msg')}
          className={`flex-1 py-2 text-sm rounded font-medium ${tab === 'msg' ? 'bg-pink-100 text-pink-700' : 'text-gray-500'}`}
        >
          💬 给老师的话
        </button>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm">
        ...
        <form onSubmit={handleSubmit}>
          <textarea
            ...
            className="w-full p-3 bg-gray-50 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 h-24 resize-none"
          ></textarea>
          <div className="flex justify-between items-center mt-2">
            <span className="text-xs text-gray-400 flex items-center">
              <Lock size={12} className="mr-1" /> 仅老师可见
            </span>
            <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm flex items-center">
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-gray-500 ml-1">我的留言记录</h3>
        {msgs.filter(m => m.type === tab).map(msg => (
          <div key={msg.id} className="bg-white p-3 rounded-xl shadow-sm border border-gray-100">
            ...
            {msg.reply && (
              <div className="bg-yellow-50 p-2 rounded-lg ml-6 text-xs text-yellow-800 border border-yellow-100">
                <span className="font-bold text-yellow-900">老师回复：</span>
                {msg.reply}
              </div>
            )}
            {!msg.reply && (
              <div className="ml-8 text-xs text-gray-400 italic">等待老师回复中...</div>
            )}
          </div>
        ))}
      </div>
